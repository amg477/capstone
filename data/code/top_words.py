...
def extract_top_terms_from_processed(
    ...
    IN_CSV  = ROOT / "data" / "files" / "processed" / "final_model_dataset_1gb.csv"
    OUT_DIR = ROOT / "data" / "files" / "processed"

    df = pd.read_csv(IN_CSV)

    # Configuration - easily change top_k here
    top_k = 5000 
    
    # --- Top unigrams from headline + body (weighted by vipr_weight) ---
    ...
    top_terms.to_csv(OUT_DIR / f"top_{top_k}_words.csv", index=False)
    ...
    top_terms_bi.to_csv(OUT_DIR / f"top_{top_k}_bigrams.csv", index=False)

    # Keyword list for attribution
    pd.Series(top_terms["word"]).to_csv(
        OUT_DIR / f"top_{top_k}_keywords.csv", index=False, header=False
    )
    print(f"Saved keyword files in {OUT_DIR}")
    print(f"Created files: top_{top_k}_words.csv, top_{top_k}_bigrams.csv, top_{top_k}_keywords.csv")