...
def create_sentiment_analysis_chart(df):
    ...
    # Group by cluster_label or cluster
    group_col = 'cluster_label' if 'cluster_label' in df.columns else 'cluster'
    
    if group_col not in df.columns:
        return None
    
    sentiment_data = df.groupby(group_col).agg({
    ...
    fig = px.scatter(
        ...
        title='Sentiment Score vs Mention Count by Cluster',
        labels={
            'sentiment_score': 'Average Sentiment Score',
            'mention_count': 'Total Mentions',
            group_col: 'Cluster Label'
        },
...
def create_visibility_chart(df):
    """Create visibility vs sentiment chart"""
    if df is None or df.empty:
        return None
    
    if 'circulation_size' not in df.columns or 'sentiment_score' not in df.columns:
        return None
    ...
    fig = px.scatter(
        ...
        hover_name='person_list',
        hover_data=['mention_count'],
        title='Circulation Size vs Sentiment Analysis',
        labels={
            'circulation_size': 'Circulation Size',
...
def create_top_individuals_chart(df, n=20):
    """Create chart of top N individuals by mention count"""
    if df is None or df.empty:
        return None
    
    top_n = df.nlargest(n, 'mention_count')
    ...
    fig = px.bar(
        ...
        y='person_list',
        orientation='h',
        color='cluster_label' if 'cluster_label' in top_n.columns else None,
        title=f'Top {n} Individuals by Mention Count',
        labels={
            'mention_count': 'Number of Mentions',
            'person_list': 'Individual'
        },
        color_discrete_sequence=px.colors.qualitative.Set3
    )
    fig.update_layout(height=max(400, n * 25), yaxis={'categoryorder': 'total ascending'})
    return fig


def create_cluster_distribution_chart(df):
    ...
    cluster_counts = df.groupby(group_col).agg({
        'mention_count': 'sum',
        'person_list': 'count'
    }).reset_index()
    cluster_counts.columns = [group_col, 'total_mentions', 'num_individuals']
    
    fig = px.bar(
        cluster_counts,
        x=group_col,
        y=['total_mentions', 'num_individuals'],
...
def create_metrics_comparison(df):
    """Create metrics comparison by cluster"""
    if df is None or df.empty:
        return None
    ...
    metrics = ['sentiment_score', 'circulation_size']
    available_metrics = [m for m in metrics if m in df.columns]
    
    if not available_metrics:
        return None
    
    cluster_means = df.groupby(group_col)[available_metrics].mean().reset_index()
...
def create_article_length_analysis(articles_df):
    """Analyze sentiment and clustering based on article length"""
    if articles_df is None or articles_df.empty:
        return None, None, None
    
    # Check for required columns
    needed_cols = ['body_token_count', 'headline_token_count']
    if not all(col in articles_df.columns for col in needed_cols):
        return None, None, None
    
    analysis_df = articles_df.copy()
    
    # Calculate total article length
    analysis_df['total_token_count'] = (
        analysis_df['body_token_count'].fillna(0) + 
        analysis_df['headline_token_count'].fillna(0)
    )
    
    # Create length categories
    if analysis_df['total_token_count'].max() > 0:
        analysis_df['length_category'] = pd.cut(
            analysis_df['total_token_count'],
            bins=[0, 500, 1000, 2000, 5000, float('inf')],
            labels=['Very Short (<500)', 'Short (500-1000)', 'Medium (1000-2000)', 'Long (2000-5000)', 'Very Long (>5000)']
        )
    
    charts = {}
    
    # Chart 1: Article Length vs Circulation
    if 'circulation_size' in analysis_df.columns:
        fig1 = px.box(
            analysis_df,
            x='length_category',
            y='circulation_size',
            title='Circulation Size by Article Length',
            labels={
                'length_category': 'Article Length (Tokens)',
                'circulation_size': 'Circulation Size'
            }
        )
        fig1.update_layout(height=400, xaxis_tickangle=-45)
        charts['circulation'] = fig1
    
    # Chart 2: Article Length vs Sentiment
    if 'sentiment_score' in analysis_df.columns:
        fig2 = px.box(
            analysis_df,
            x='length_category',
            y='sentiment_score',
            title='Sentiment Score by Article Length',
            labels={
                'length_category': 'Article Length (Tokens)',
                'sentiment_score': 'Sentiment Score'
            }
        ...
        charts['sentiment'] = fig2
    
    # Chart 3: Article Length Distribution by Cluster
    cluster_col = 'cluster_label' if 'cluster_label' in analysis_df.columns else 'cluster'
    if cluster_col in analysis_df.columns:
        length_cluster = analysis_df.groupby([cluster_col, 'length_category']).size().reset_index(name='count')
        fig3 = px.bar(
            length_cluster,
            x='length_category',
            y='count',
            color=cluster_col,
            title='Article Length Distribution by Cluster',
            labels={
                'length_category': 'Article Length (Tokens)',
    ...
    # Summary statistics
    agg_dict = {'total_token_count': ['mean', 'median']}
    if 'circulation_size' in analysis_df.columns:
        agg_dict['circulation_size'] = ['mean', 'median', 'count']
    if 'sentiment_score' in analysis_df.columns:
        agg_dict['sentiment_score'] = ['mean', 'median']
    
    if agg_dict:
        summary = analysis_df.groupby('length_category').agg(agg_dict).round(2)
    else:
        summary = pd.DataFrame()
    
    return charts, summary, analysis_df
